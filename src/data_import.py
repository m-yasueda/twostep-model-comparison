...
#
# Consolidated from the three identical copies of WTdata_import.py that
# previously lived in OIST/, OIST/BehavioralSimulation_code/, and
# OIST/CrossValidation/.
#
# Usage (from any notebook in organized/notebooks/):
#     import sys, os
#     sys.path.insert(0, os.path.join(os.path.dirname('__file__'), '..'))
#     from src.data_import import Session, Experiment, load_analog_data
...
class Session():
    ...
    def __init__(self, file_path, int_subject_IDs=True):
        ...
        value_lines = [line[2:] for line in all_lines if line[0]=='V']
        self.training_stage = next(line for line in value_lines if 'training_stage' in line).split(' ')[2]
